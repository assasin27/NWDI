...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE policyname = 'Farmers can view their own profile'
        AND tablename = 'farmer_profiles'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Farmers can update their own profile'
        AND tablename = 'farmer_profiles'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Products are viewable by everyone'
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Products are insertable by farmers'
        AND tablename = 'products'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Products are updatable by farmers'
        AND tablename = 'products'
    ) THEN
